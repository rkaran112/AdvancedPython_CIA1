...
- `test_state_utils.py` covers the state-name normalization helper, and `test_trend_utils.py` covers the January Trends weekly breakdown/growth calculation (`unittest`, run with `python -m unittest discover`). The rest of `streamlit.py` still has no automated tests.
