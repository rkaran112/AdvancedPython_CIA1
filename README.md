...
- `test_state_utils.py` covers the state-name normalization helper, `test_trend_utils.py` covers the January Trends weekly breakdown/growth calculation, `test_price_utils.py` covers the Historical Prices tab's CSV date parsing and price-range filter, and `test_calculator_utils.py` covers the Price Calculator tab's weight/currency conversion (`unittest`, run with `python -m unittest discover`). The rest of `streamlit.py` still has no automated tests.
